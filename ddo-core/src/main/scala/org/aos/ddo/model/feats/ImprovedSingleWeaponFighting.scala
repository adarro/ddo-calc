...
/** Icon Feat Single Weapon Fighting.png
  ...
  * Base Attack Bonus +6
  * */
protected[feats] trait ImprovedSingleWeaponFighting extends FeatRequisiteImpl with Passive with RequiresAllOfFeat with RequiresSkill with RequiresBaB {
  self: GeneralFeat =>
  override def requiresBaB = 6

  override def requiresSkill: Seq[(Skill, Int)] = List((Skill.Balance, 4))

  override def allOfFeats: Seq[GeneralFeat] = List(GeneralFeat.SingleWeaponFighting)
}
