...
object Skill extends Enum[Skill] with SearchPrefix {
  ...
  /**
    * Allows you to bluff certain NPCs, draw monsters away from their group,
    * or make an individual monster vulnerable to sneak attack for six seconds.
    * You also gain a decrease in threat generation for a short period of time after successfully using this ability.
    * (15 second cooldown, Shares cooldown with Diplomacy)
    */
  ...
  /**
    ...
    * Any accumulated threat you have with enemies that you successfully diplomacize is reduced. (15 second cooldown, Shares cooldown with Bluff)
    */
  case object Diplomacy extends Skill with AtWillEvent with CharismaLinked

  /**
    * Can be used to disarm traps in dungeons.
    * Requires that the control box of the trap has been located by a successful Search check.
    * If the Disable Device check succeeds, you disable the trap.
    * If it fails by 4 or less, you have failed but can try again.
    * If you fail by 5 or more the control box explodes
  ...
  /**
    ...
    * Also, the character with the highest Heal skill is used when using Rest shrines to dictate how much health is recovered.
  ...
  /**
    ...
    * You will suffer a -4 penalty to your intimidate skill for every size category you are smaller than
    * the creature your are trying to intimidate, or +4 for every size category your are larger.
  ...
  /**
    * Allows you to jump higher. This skill has a maximum cap of 40. 9th level and higher casters can cast +30 Jump (spell)
    */
  case object Jump extends Skill with PassiveActivation with StrengthLinked

  /**
    * Allows you to hear approaching enemies.
    * You will be notified by red pulses (footsteps) as they approach.
    * There are certain quests that utilize the listen check for floor-crumbling traps (i.e. The Chamber of Raiyum).
    */
  ...
  /**
    ...
    * This skill is required for bards to be able to use their Bardic Music.
    * Each point of skill adds 1 point to Sonic Spell Power.
    */
  case object Perform extends Skill with PassiveActivation with CharismaLinked

  /**
    * Allows you to restore a disabled warforged character to 1 hit point
    * (each attempt requires one repair kit).
  ...
  /**
    ...
    * Note that (like Spot) there is no die roll involved.
    * If your score is high enough you will always succeed and if it's too low you'll never find something.
    * (1 second cooldown)
    */
  ...
  /**
    * Tumbling Allows you to tumble and roll away from attacking enemies.
    * Tumbling can be accomplished by blocking and using a movement key.
    * A skill rating higher than 30 allows you to dive to the side and do back flips instead of rolling.
    * Skill also reduces falling damage.
    */
