...
object Requirement extends Enum[Requirement] {
  ...
  private def alignments = Alignments.values.map { x => ReqAlignment(Right(x)) } ++
    ...
    MoralAxis.values.map { x => ReqAlignment(Left(x)) }

  private def skills = Skill.values.map { x => ReqSkill(id = x.entryName, amount = 0) }
  ...
  case class ReqActionPoint(amount: Int) extends Requirement {
    ...
    /**
      * @inheritdoc
      */
    override protected def nameSource: String = "ActionPoint".splitByCase.toPascalCase
  }
  ...
  case class BaseAttackBonus(amount: Int) extends Requirement with Abbreviation {
  ...
  case class ReqAlignment(id: Either[AlignmentType, Alignments]) extends Requirement {
    ...
    override protected def nameSource: String = id match {
      case Left(x: LawAxis) => s"Align:{${x.entryName.splitByCase.toPascalCase}}"
      case Left(x: MoralAxis) => s"Align:{${x.entryName.splitByCase.toPascalCase}}"
