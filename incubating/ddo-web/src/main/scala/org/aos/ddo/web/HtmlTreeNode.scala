...
/** @author Andre White
  * Represents a TreeNode like list derived from an HTML fragment.
  *
...
/** Represents an end node with no branches
  * @param text the value of the leaf
  */
...
/** Represents a node that can have additional leaves and branches
  * @param branches nested branches, if any
  * @param leaves any leaves on the branch
  *
  */
case class Branch(branches: Option[Iterable[Branch]] = None, leaves: Option[Iterable[Leaf]] = None) extends HtmlTreeNode {
  val isStump = false
}
...
/** @author Andre White
  * Represents an object with no leaves or branches.
  */
trait UnBranchable {
