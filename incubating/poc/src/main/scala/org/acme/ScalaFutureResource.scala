...
class ScalaFutureResource():
  /**
   * This is an endpoint which demonstrates how to perform asynchronous operations. If you call
   * `Await.result` inside this method, annotate it with @Blocking to make Quarkus move the method
   * to a separate thread pool meant for blocking operations. Otherwise (as is demonstrated here)
   * return a `CompletionStage` which can be obtained from Scala Future by using
