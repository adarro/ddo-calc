...
object StringUtils extends LazyLogging {
  ...
  implicit class StringImprovements(val s: String) {
     ...
     *
     * @return
     *   Some(Int) upon a successful conversion or None for a failed attempt.
     */
  ...
  implicit class IntExtensions(val source: Int) {
    ...
    /**
     * Convenience toString to add +/- to the number for text fields
     * @return
     *   returns the number as a string prefixed with either a "+" or "-"
     */
  ...
  implicit class Extensions(val s: String) {
     ...
     * @note
     *   This utility parses using Space as a delimiter. It is generally advised to use this before
     *   other manipulations such as removing spaces or changing cases.
     * @return
