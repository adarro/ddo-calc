...
/**
 * [[http://ddowiki.com/page/Uncanny_Dodge Uncanny Dodge]] This feat grants you a 1% passive bonus to Dodge at levels 4,
 * 6, 8, 12, 16, and 20. Also, you can activate this ability to gain a temporary 50% dodge bonus and a +6 reflex save
 * bonus. As of Update 14, using this ability is no longer restricted by number of uses per rest. However, it is
 * restricted by cooldown.
 *
 * @note
 *   This feat is replaced by [[ImprovedUncannyDodge]] if character has both. also available via Swashbuckler
 *   Enhancements
 */
protected[feats] trait UncannyDodge
  extends FeatRequisiteImpl with Passive with ActiveFeat with AtWillEvent with GrantsToClass with RequiresAnyOfClass {
  self: ClassFeat =>
