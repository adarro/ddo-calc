 ...
 *
  ...
  /** Handedness
    * Represents how a weapon can be used / equipped. i.e. Daggers (MustContainAtLeastOne Handed, Longbow  two handed)
    *
  ...
  /** handwrap example
    ...
    * Handedness: MustContainAtLeastOne-handed weapon
    * Default Modifiers
    * Damage: STR
    * Attack: STR
...
/** Basic trait for allowing an item to be wielded using one or more hand slots.
  ...
  * @todo  Bows / x-bows equip to main hand, need a plug or otherwise to 'occupy' the off-hand slot
  *
  */
...
trait OneHandedWeapon extends HandedWeapon {
  lazy val allowedWearLocations: Seq[EnumEntry] = {
    WearLocation.fromMask(
      WearLocation.MainHand.bitValue | WearLocation.OffHand.bitValue) match {
      case Some(x) => x
      case _       => Nil
    }
  }
