...
import io.truthencode.ddo.support.matching.{
  WordMatchStrategies,
  WordMatchStrategy
}
  ...
  object StringUtils {
    ...
    implicit class Extensions(val s: String) {
      def lowerCaseNoise: String = {
        val noise =
          config.getStringList("io.truthencode.ddo.support.noiseWords")
      ...
      /**
        * Formats string into [[https://wiki.c2.com/?CamelCase Lower Camel Case]]
        * @return camelCase formated string
        */
      def toLowerCamelCase: String = {
        val ss = s.toPascalCase
        val sc = ss.charAt(0)
        ss.replaceFirst(sc.toString, sc.toLower.toString)
      }
      ...
      /**
        ...
        */
      def wordsToAcronym(
          implicit wordMatchStrategy: WordMatchStrategy =
            WordMatchStrategies.IgnoreCaseWordStrategy
      ): Option[String] =
    ...
    private def characterToStrategy(
        c: Char,
        wordMatchStrategy: WordMatchStrategy
    ): Char = {
